...
fn create_nxo(
    ...
    match format {
        "nro" => {
            ...
            nxo.write_nro(&mut out_file, romfs_dir, icon_file.as_deref(), nacp_input)
                .map_err(|err| (err, output_file))?;
        }
        "nso" => {
            let mut out_file = output_option
...
fn process_args(app: &Opt) {
    let res = match app {
        ...
        Opt::Npdm {
            ...
            ref acid_file,
            ref output_file
        } => create_npdm(input_file, to_opt_ref(acid_file), to_opt_ref(pem_file), output_file)
    };
